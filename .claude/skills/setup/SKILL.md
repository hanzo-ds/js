...
name: setup
description: >
  Set up the `datastore-js` repository in a fresh checkout so the agent can run
...
## 3. Start Datastore (only for integration tests / examples)
...
This starts both the single-node setup (`datastore` on 8123/9000, `datastore_tls` on 8443/9440) and the two-node cluster (`datastore1`, `datastore2`, plus the `nginx` round-robin entrypoint on 8127). All services use non-overlapping ports so a single `up -d` covers every integration test mode.

To override the server version, set `DATASTORE_VERSION` when starting Compose; for example: `DATASTORE_VERSION=head docker compose up -d`, `DATASTORE_VERSION=latest docker compose up -d`, or `DATASTORE_VERSION=24.8 docker compose up -d` to use an explicit version tag.
