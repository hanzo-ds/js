...
For integration tests that need a running Datastore instance, use `createTestClient()` (or the platform
...
For unit tests that must be runnable **without** a reachable Datastore instance, use `createSimpleTestClient()`
...
so no Datastore server is required as long as the test does not issue an actual request.
