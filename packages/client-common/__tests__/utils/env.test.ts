...
import { getDatastoreTestEnvironment, TestEnv } from "./test_env";

describe("Test env variables parsing", () => {
  describe("DATASTORE_TEST_ENVIRONMENT", () => {
    const key = "DATASTORE_TEST_ENVIRONMENT";
    addHooks(key);

    it("should fall back to local_single_node env if unset", async () => {
      expect(getDatastoreTestEnvironment()).toBe(TestEnv.LocalSingleNode);
    });
    ...
    it("should be able to set local_cluster env", async () => {
      ...
      expect(getDatastoreTestEnvironment()).toBe(TestEnv.LocalCluster);
    });

    it("should be able to set cloud env", async () => {
      process.env[key] = "cloud";
      expect(getDatastoreTestEnvironment()).toBe(TestEnv.Cloud);
    });

    it("should throw in case of an empty string", async () => {
      process.env[key] = "";
      expect(getDatastoreTestEnvironment).toThrowError();
    });
  ...
  describe("DATASTORE_TEST_CONNECTION_TYPE", () => {
    const key = "DATASTORE_TEST_CONNECTION_TYPE";
    addHooks(key);
