...
# Datastore Node.js Client — Coding
...
3. **Prefer `JSONEachRow` for typical row inserts/selects** unless the user
   ...
   **Note on `datastore_settings`:** settings passed to `createClient` are
   defaults for every request; they can be overridden per-call by passing
   `datastore_settings` directly to `insert()`, `query()`, or `command()`.
...
6. **Note version constraints** when relevant. Examples:
   ...
   - `Time` / `Time64` data types: Datastore server `>= 25.6`.
   - `QBit` data type: Datastore server `>= 25.10` (GA on `26.x`).
   - `Dynamic` / `Variant` / new `JSON` types: Datastore server `>= 24.1` /
...
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| **Configure / connect the client**                       | Building a `createClient` call, URL parameters, `datastore_settings`, default format, custom HTTP headers                                                                                                                       | `reference/client-configuration.md` |
...
| **Ping the server**                                      | Health checks, readiness probes, "is Datastore up?"                                                                                                                                                                             | `reference/ping.md`                 |
...
- When showing parameter binding, use Datastore's native `{name: Type}`
  syntax — never `$1`, `?`, or `:name`.
- For DDL inside a cluster or behind a load balancer, set
  `datastore_settings: { wait_end_of_query: 1 }` on the `command()` call so
  the server only acknowledges after the change is applied. See
  https://docs.hanzo.ai/datastore/en/interfaces/http/#response-buffering.
...
- **Errors, hangs, type mismatches, proxy pathname surprises, log silence,
  ...
  `datastore-js-node-troubleshooting` skill.
- **Streaming, Parquet, file streams, server-side bulk moves, progress
  streaming, async-insert throughput tuning** — see
  [`examples/node/performance/`](https://github.com/hanzo-ds/js/tree/main/examples/node/performance).
- **TLS, RBAC / read-only users, deeper SQL-injection guidance** — see
  [`examples/node/security/`](https://github.com/hanzo-ds/js/tree/main/examples/node/security).
...
- [`examples/node/coding/`](https://github.com/hanzo-ds/js/tree/main/examples/node/coding) — the runnable corpus this skill is built on.
- [Datastore JS client docs](https://docs.hanzo.ai/datastore/integrations/javascript)
- [Datastore supported formats](https://docs.hanzo.ai/datastore/interfaces/formats)
- [Datastore data types](https://docs.hanzo.ai/datastore/sql-reference/data-types)
