...
> batching layer. Datastore will batch them server-side. This is also the
> recommended ingestion pattern for **Datastore Cloud**.
  ...
  await client
    ...
    .catch((err) => {
      if (err instanceof DatastoreError) {
        ...
        console.error(`Datastore error ${err.code}:`, err);
        return;
      }
...
- **Not handling `DatastoreError`.** It exposes `err.code`, which maps to
...
- For raw throughput tuning of large async-insert workloads,
  see [`examples/node/performance/`](https://github.com/hanzo-ds/js/tree/main/examples/node/performance).
