...
> - `datastore_setting_*` / `ch_*` URL parameters: client `>= 1.0.0`.
...
- When mentioning the URL form for environment variables / DSNs: show a **Bash**
  `export` with the literal URL value, and `createClient({ url: process.env.DATASTORE_URL })`
...
- If `datastore_settings` appear on `createClient`, explain that they are
...
const client = createClient({
  ...
  username: process.env.DATASTORE_USER, // defaults to 'default'
  password: process.env.DATASTORE_PASSWORD, // defaults to ''
...
export DATASTORE_URL='https://bob:secret@my.host:8124/analytics'
```

```ts
// In your Node.js code — no URL construction needed:
const client = createClient({ url: process.env.DATASTORE_URL });
```

## Per-client vs per-request `datastore_settings` ⭐

> **Always mention this when discussing `datastore_settings`:** settings set
...
const { stream, query_id } = await client.exec({
  ...
  datastore_settings: { default_format: format },
});
