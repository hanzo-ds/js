...
>
> - `Variant`: Datastore `>= 24.1`.
> - `Dynamic`: Datastore `>= 24.5`.
> - New `JSON` (object) type: Datastore `>= 24.8`.
...
> - `Time` / `Time64`: Datastore `>= 25.6` and require
>   `enable_time_time64_type: 1`.
> - `QBit`: Datastore `>= 25.10` (experimental, gated by
...
- Use the new `JSON` column type, introduced in Datastore `>= 24.8`.
- Say `JSON` is no longer experimental since Datastore `25.3`; on older
...
- **State the version policy in your explanation AND inline in the code.**
  ...
  ```ts
  datastore_settings: {
    // JSON type introduced in Datastore 24.8, non-experimental since 25.3.
...
```ts
const client = createClient({
  datastore_settings: { enable_time_time64_type: 1 },
});
...
Introduced in Datastore `25.10` as an experimental type (gated by
...
Datastore handle the bit-plane layout — don't feed raw `FixedString` bytes
...
const client = createClient({
  datastore_settings: {
    // QBit introduced in Datastore 25.10 (experimental), GA since 26.x.
...
- **Targeting old Datastore servers without the `allow_experimental_*`
