...
> Datastore feature; the JSON _formats_ listed here are universally supported
> by the client.
...
- **Use runtime type checkers like `zod` or `io-ts` if your app ingests untrusted JSON.**
  It's easier to debug mismatches between your data and the format's expected shape with a validation library used at the place of ingestion than with Datastore errors, especially in the middle of a large insert batch or streaming operation.
