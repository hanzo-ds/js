...
> setting available on every supported Datastore version.

## `INSERT … SELECT` (no values payload)

When the data already lives in Datastore, use `client.command()` with a raw
`INSERT … SELECT`:
...
Datastore `{name: Type}` parameters. Run it via `command()` and set
...
Datastore converts a `UUID` into `UInt128` implicitly **only for the `VALUES`
