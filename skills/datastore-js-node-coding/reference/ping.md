...
When answering "how do I health-check / readiness-probe Datastore?":
...
if (pingResult.success) {
  console.info("Datastore is reachable");
} else {
...
- Probe Datastore at application startup.
- Wake up a Datastore Cloud instance that may be idling (a ping is enough to
...
The default `ping()` hits Datastore's `/ping` HTTP endpoint — it verifies
...
- **Waking a Datastore Cloud idle instance** — plain `ping()` is enough.
...
- **Only ping the Datastore server in your app's liveness probe** if the app
  has to be restarted to recover from a Datastore outage. If the app can recover
  the connection to Datastore without a restart, put the ping in a readiness
