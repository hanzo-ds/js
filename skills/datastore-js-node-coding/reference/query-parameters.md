...
- Use Datastore `{name: Type}` placeholders and a `query_params` object.
...
- Pick the placeholder type to match the Datastore column type (`String`,
  `Date`, `DateTime`, `Nullable(T)`, etc.).

## Syntax: `{name: Type}`

Datastore uses `{name: Type}` placeholders — **not** `$1`, `?`, or `:name`.
...
The `Type` must be a valid Datastore type (`Int32`, `String`, `Date`,
