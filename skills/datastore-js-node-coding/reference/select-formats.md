...
numbers instead. Note that in Datastore `>= 25.8`, this setting can
default to `0`; see the troubleshooting skill for ways to control that.

## Single-document `JSON` format with metadata

Use `JSON` (or `JSONCompact`) when you need Datastore's response envelope
...
[`select_parquet_as_file.ts`](https://github.com/hanzo-ds/js/blob/main/examples/node/performance/select_parquet_as_file.ts)).
...
| Stream very large result                                 | `JSONEachRow` / `JSONCompactEachRow` (see [`examples/node/performance/`](https://github.com/hanzo-ds/js/tree/main/examples/node/performance)) |
| Export to CSV/TSV/Parquet                                | `CSV*`, `TabSeparated*`, `Parquet` (see [`examples/node/performance/`](https://github.com/hanzo-ds/js/tree/main/examples/node/performance))   |
...
| `rs.stream()`        | Node `Readable` of `Row[]` chunks                | Use for large newline-delimited results (`JSONEachRow`/`JSONCompactEachRow`/`CSV`/`TSV`); **not** suitable for binary formats like `Parquet` — for those, use `client.exec()` and pipe the raw response stream (see [`examples/node/performance/`](https://github.com/hanzo-ds/js/tree/main/examples/node/performance)) |
