...
- For Datastore Cloud or any load-balanced deployment: **explicitly
...
- Any other server feature scoped per session (e.g., session-scoped
  variables in newer Datastore versions).

## ⚠️ `session_id` and concurrency

Datastore **rejects concurrent queries within the same session** — if two
...
await client.command({
  ...
  datastore_settings: { wait_end_of_query: 1 }, // ack before next call
});
...
## ⚠️ Sessions and load balancers / Datastore Cloud

Sessions are bound to a **specific Datastore node**. If a load balancer in
front of Datastore routes consecutive requests to different nodes, the
temporary table / `SET` won't be visible — you'll get
`UNKNOWN_TABLE` / surprising results.

Mitigations (in order of preference):

- **For Datastore Cloud, use [replica-aware
  routing](https://docs.hanzo.ai/datastore/manage/replica-aware-routing)** so
