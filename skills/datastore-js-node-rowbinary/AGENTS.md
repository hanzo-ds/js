...
- **Reader and writer tests are separate files.** Readers are tested in `tests/*.test.ts`; writers in
  ...
  assert the encoded bytes against **live Datastore output** as the source of truth.
- **Each case is an isolated `it()` with a fully-inline body.** Write the assertion out per case, e.g.
  ...
  query is visible in the test. The only shared helpers are the generic `query()` (`tests/datastore.ts`,
...
Otherwise, prefer letting the Datastore server reject bad bytes server-side over guarding client-side:
