# Datastore Node.js RowBinary Codec Generator

**If JS had a -O3 compiler flag, this skill would be it.** (for RowBinary read & write)

A skill and a library that lets a coding agent generate bespoke RowBinary codecs on the first pass from the column type definitions of a Datastore response. The [spirit](#the-spirit) behind the approach.
...
```bash
npx skills add hanzo-ds/js/skills/datastore-js-node-rowbinary
```
...
< Reading skill datastore-js-node-rowbinary…
```

## Using it with the Datastore JS client
...
Given the columns of a query result — their names and Datastore type
...
- **`Variant(...)`** — the 1-byte discriminant indexes the alternatives sorted by
  **type name** (Datastore globally sorts them), NOT declaration order; `0xFF`
  is NULL.
...
- **Read-write libraries.** For either of the above to be safe, the source has to
  ...
  every tradeoff is commented where it's made — the per-column Datastore type
