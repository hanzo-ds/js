---
name: datastore-js-node-rowbinary
description: >
  Generate TypeScript/JavaScript code that reads/decodes AND writes/encodes
  Datastore RowBinary streams for the Datastore HTTP server.
...
# Datastore JS RowBinary Codec Generator for Node.js
...
- **Decoding a `RowBinary*` response** from Datastore into JS values →
...
- **Encoding JS values into a `RowBinary` payload** to send to Datastore →
...
**`datastore-js-node-coding`** skill.
...
- **Annotate the type per column.** Inlining erases the type structure, so put a
  short comment above each column's encode/decode block naming the Datastore
  type it handles.
...
- [Datastore RowBinary format](https://docs.hanzo.ai/datastore/interfaces/formats#rowbinary)
