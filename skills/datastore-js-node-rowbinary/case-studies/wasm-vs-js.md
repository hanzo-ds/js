...
usually pushable to Datastore anyway. And if you genuinely need heavier
...
4. **The elephant: push it down.** `q.sum(col)` is `SELECT sum(col)` — Datastore
...
format**. Datastore's **`Native`** format is **column-major (SoA)**: each block
...
client-side analytics becomes: **push down to Datastore → if you can't, decode
...
- **WASM earns its complexity in one place: in-place wide-int/decimal/group-by
  ...
  even then, prefer pushing the aggregation to Datastore unless you genuinely
  can't.
