...
/**
 * Parse a binary TYPE ENCODING (the header Datastore writes before each
 ...
 *
 * Only the tags Datastore actually emits for stored `Dynamic` values are
...
export function readDynamicType(state: Cursor): Reader<unknown> {
  ...
  switch (tag) {
    ...
    // encoding). Datastore writes the alternatives ALREADY SORTED by type name,
