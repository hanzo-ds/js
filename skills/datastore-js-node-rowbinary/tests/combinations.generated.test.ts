 ...
 * context. Datastore is the byte oracle — a case only declares its SQL
 ...
 * Datastore expression (every level casts, so server-side type inference is
...
/**
 * Hold `chosen` inside a `Variant(chosen, marker)`. Datastore sorts alternatives
