...
> **Applies to:** all versions. The `output_format_json_quote_64bit_integers` Datastore setting is server-side and can be passed via `datastore_settings` in any client version.
...
const resultSet = await client.query({
  ...
  format: "JSONEachRow",
  datastore_settings: { output_format_json_quote_64bit_integers: 0 },
});
```
...
> **Applies to:** all versions (this is a Datastore JSON serialization behavior). For custom JSON parse/stringify (e.g., using a BigInt-safe parser), see `>= 1.14.0` which added configurable `json.parse` and `json.stringify` functions.

Datastore returns Decimals as numbers by default in `JSON*` formats. Cast to string in the query:
...
> **Applies to:** all versions. This is a Datastore input-parsing behavior, not a client bug.

Datastore converts a `UUID` into `UInt128` implicitly **only for the `VALUES` clause**. With the row-oriented JSON formats the client uses (e.g. `JSONEachRow`), sending a UUID string such as `'019982cb-3abf-7e12-9668-c788a9e3639c'` for a `UInt128` column fails with `CANNOT_PARSE_INPUT_ASSERTION_FAILED`.
...
**Pattern 2 — declare the UUID column as `EPHEMERAL`** and let Datastore populate the `UInt128` column via its `DEFAULT` expression. The ephemeral column must be listed in `columns` so the `DEFAULT` is evaluated:
