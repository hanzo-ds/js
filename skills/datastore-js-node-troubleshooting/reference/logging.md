...
> **Requires:** `>= 0.2.0` (explicit `log.level` config option introduced in 0.2.0, replacing the `DATASTORE_LOG_LEVEL` env var from 0.0.11). Custom `LoggerClass` also available since `>= 0.2.0`. In `>= 1.18.1`, the default changed from `OFF` to `WARN` and logging became lazy (messages only constructed if the log level matches). In `>= 1.18.1`, structured context fields (`connection_id`, `query_id`, `request_id`, `socket_id`) are available in logger `args`.
...
const client = createClient({
  log: {
    level: DatastoreLogLevel.DEBUG, // TRACE | DEBUG | INFO | WARN | ERROR
  },
});
```
...
const client = createClient({
  log: { LoggerClass: MyLogger, level: DatastoreLogLevel.INFO },
});
```
