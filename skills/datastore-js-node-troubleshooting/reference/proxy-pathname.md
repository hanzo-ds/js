...
**Symptom:** Wrong database is selected, or requests fail when Datastore is behind a proxy with a path prefix (e.g., `http://proxy:8123/datastore_server`).
...
const client = createClient({
  url: "http://proxy:8123",
  pathname: "/datastore_server", // leading slash optional; multiple segments supported
});
```
