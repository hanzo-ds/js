...
When `$1`/`?` don't work, a common instinct is to interpolate values directly with a template literal. Don't — this bypasses Datastore's server-side escaping and opens the door to SQL injection:
...
The Datastore JS client uses Datastore's native `{name: type}` syntax — not `$1`/`?`/`:name` placeholders from other databases:
