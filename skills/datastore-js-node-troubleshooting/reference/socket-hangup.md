...
```js
import { createClient, DatastoreLogLevel } from "@hanzo/datastore-client";
...
```bash
curl -v --data-binary "SELECT 1" <your_datastore_url>
```
...
The default `idle_socket_ttl` in the client is **2500 ms**, which is safe for servers with a 3 s timeout (common in Datastore < 23.11). If your server has a higher timeout (e.g., 10 s), you can safely increase:
...
> **Note:** `http_headers_progress_interval_ms` is a `UInt64` Datastore setting, so it must be passed as a **string** (e.g., `'10000'`).
