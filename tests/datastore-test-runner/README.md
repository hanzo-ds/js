...
harness. It wraps [`@hanzo-ds/client`](../../packages/client-node) in a tiny CLI that
...
through `@hanzo-ds/client`, without having to reimplement the test runner
itself.
...
- **`passthrough`** (default) — set `default_format = TabSeparated` and stream
  ...
  formatting; this exercises `@hanzo-ds/client`'s transport, session and
...
- **`rowbinary`** — for each result-returning statement that has no explicit
  ...
  published [`@hanzo-ds/rowbinary`](https://www.npmjs.com/package/@hanzo-ds/rowbinary)
...
The `rowbinary` backend depends on the published `@hanzo-ds/rowbinary` package
...
installs alongside (and is linked against) the local `@hanzo-ds/client` and
`@hanzo-ds/client-common` packages. Install + build everything from the
repo root:
...
compiles `@hanzo-ds/client` and `@hanzo-ds/client-common` so the harness
...
| -------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TEST_RUNNER_BACKEND`      | `passthrough`                                                    | Which backend executes queries: `passthrough` (stream Datastore's own `TabSeparated`) or `rowbinary` (decode `RowBinaryWithNamesAndTypes` via `@hanzo-ds/rowbinary` and re-render). See [Backends](#backends). |
...
`@hanzo-ds/client` and `@hanzo-ds/client-common` are linked from the
