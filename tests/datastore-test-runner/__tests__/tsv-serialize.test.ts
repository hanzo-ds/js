...
describe("renderValue — top level (escaped, unquoted)", () => {
  ...
  it("float64 signed zero and exponent style match Datastore", () => {
  ...
  it("float32 renders the shortest single-precision round-trip, not the widened double", () => {
    // Math.fround(0.26894) === the value Datastore decodes; String() of that
    // double is 0.2689400017261505, but Datastore prints the shortest form.
  ...
  it("uuid / ipv4 / ipv6 from their decoded byte/number forms", () => {
    // readUUID hands back the raw 16 wire bytes; formatUUID applies Datastore's
