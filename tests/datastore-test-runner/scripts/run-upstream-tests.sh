...
UPSTREAM_DATASTORE_DIR="${UPSTREAM_DATASTORE_DIR:-${RUNNER_DIR}/.upstream/Datastore}"
DATASTORE_CLIENT_CLI_LOG="${DATASTORE_CLIENT_CLI_LOG:-${RUNNER_DIR}/.upstream/datastore-client-cli.log}"
UPSTREAM_TEST_LIST="${UPSTREAM_TEST_LIST:-${RUNNER_DIR}/upstream-allowlist.txt}"

# Verify upstream Datastore directory
if [[ ! -x "${UPSTREAM_DATASTORE_DIR}/tests/datastore-test" ]]; then
  echo "Error: ${UPSTREAM_DATASTORE_DIR}/tests/datastore-test not found or not executable." >&2
  echo "Set UPSTREAM_DATASTORE_DIR to point to a checkout of Datastore/Datastore." >&2
  exit 1
fi
...
# to tests/datastore-test even if the allowlist file is hand-edited.
tests=()
...
if [[ ${#tests[@]} -eq 0 ]]; then
  if [[ "${ALLOW_EMPTY_UPSTREAM_ALLOWLIST:-0}" != "1" ]]; then
    ...
    echo "Refusing to run tests/datastore-test without explicit test names because an empty allowlist can run a large upstream suite." >&2
...
mkdir -p "$(dirname "${DATASTORE_CLIENT_CLI_LOG}")"

# Export environment for the wrapper
export PATH="${RUNNER_DIR}/bin:${PATH}"
export DATASTORE_CLIENT_CLI_LOG

# Run the upstream test runner
cd "${UPSTREAM_DATASTORE_DIR}"
exec python3 tests/datastore-test "${tests[@]}" "$@"
