...
/** Server settings to forward, assembled the same way for every backend. */
export function buildDatastoreSettings(
  args: ParsedArgs,
...
/**
 * Create a client bound to a dedicated per-invocation `session_id` so that
 * settings applied via `SET ...` in one statement persist for subsequent
 * statements within the same .sql script. Without a session, every
 * `client.exec(...)` is an independent HTTP request and `SET` has no effect on
 * later requests, which breaks upstream tests that rely on patterns like
...
export function createSessionClient(
  ...
  const sessionId = `datastore-js-test-runner-${randomUUID()}`;
  ...
  return createClient({
    ...
    // ... }` hint), and upstream `datastore-test` fails any test that writes to
    // stderr — so keep the client itself silent.
    log: { level: DatastoreLogLevel.OFF },
  });
}

/**
 * Reconcile the outcome of a single statement against its expected-error hint,
 * the same way for every backend:
 *   - hinted (`-- { serverError ... }`): an error is the success path, its
 *     absence (or a mismatching error) is a failure;
 *   - un-hinted: any error propagates.
 * Throws on failure; returns normally when the statement is considered passed.
 */
export function settleExpectedError(
  stmt: Statement,
  execError: unknown,
  logPath: string,
): void {
