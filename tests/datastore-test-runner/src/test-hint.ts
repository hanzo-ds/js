// Support for upstream `datastore-test` error hints.
//
// Many `.sql` tests in Datastore/Datastore annotate a statement that is
...
//
// The real `datastore-client` parses these hints (see upstream
...
// otherwise the propagated `DatastoreError` aborts the script and the test is
...
export interface ExpectedError {
  ...
  label: string;
  /** Numeric error codes (as strings), matched against `DatastoreError.code`. */
  codes: Set<string>;
  /** Named error codes, matched against `DatastoreError.type`. */
  names: Set<string>;
}
