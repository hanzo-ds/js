 ...
 * Datastore produces for the `TabSeparated` format, so the upstream
 * `datastore-test` runner can diff our output against the checked-in
 * `.reference` files.
 *
 * This is the value-oracle half of the RowBinary backend: Datastore is the
 ...
 *
 * Two text contexts, mirroring Datastore's `serializeTextEscaped` (top level)
...
// C-style escapes for the control characters Datastore escapes in text
...
/** Datastore's lower-case words for the non-finite floats and signed zero, else null. */
...
/**
 * Reconcile JS's number text with Datastore's: Datastore writes a positive
...
/**
 * Datastore `Float64` text: the shortest decimal that round-trips to the
...
/**
 * Datastore `Float32` text. The parser widens a Float32 to a JS double, so
 * `String(n)` would print the double's full precision (e.g. `0.2689400017261505`
 * for a value Datastore prints as `0.26894`). Datastore instead emits the
...
function formatDateTime64(value: unknown, precision: number): string {
  ...
  // TRUNCATE toward the precision, matching Datastore's text output — never
...
/**
 * Render one decoded value as Datastore TSV text. `nested` selects the quoted
...
export function renderValue(
  ...
  switch (node.name) {
    ...
    // --- geo: fixed nestings of Point (a Float64 pair). Datastore renders
